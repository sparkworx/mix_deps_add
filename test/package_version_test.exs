defmodule PackageVersionTest do
  ...
  test "Finds the current version of a remote package" do
    assert {:versioned, "1.3.0-rc.0"} = PackageVersion.current("phoenix", FakeHexClient)
  end
  ...
  test "Returns an error if a remote package isn't found in Hex" do
    assert {:error, :notfound} = PackageVersion.current("sdlfkjsdfiuoiusdflks", FakeHexClient)
  end

  test "Returns an error if a local package path doesn't exist" do
    assert {:error, :nonexistant} = PackageVersion.current("../does_not_exist", :unused)
  end
end
